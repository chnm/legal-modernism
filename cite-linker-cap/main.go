...
// cite-linker-cap links the citations cite-detector-cap detected in the text
// of CAP opinions (opinion_citations.citations_unlinked) to cases, writing
// opinion_citations.citation_links (issue #74). It is cite-linker over the CAP
// ledger: the cascade, the lookup tables, the flags and the exit codes are the
// same, and only the corpus store differs. A citation's citing document is its
// opinion's case, so the anachronism rule (issue #319) refuses a hit on a case
// decided after that case; a case citing itself is of the same year and links.
...
func main() {
	...
	slog.Info("starting the CAP citation linker")
	...
	// The lookup tables are the ones cite-linker loads; only the ledger differs:
	...
	// from psql and then running this program unchanged; the anti-join in
	// StreamUnprocessedCitations then resumes from wherever a previous job
	// stopped. The stub registry (legalhist.stub_cases) is read here as it is
	// there, but it is built from the MOML linker's misses alone: a citation in
	// an opinion to a reporter no source covers links to a stub the treatises
	// minted, or stays no_match.
