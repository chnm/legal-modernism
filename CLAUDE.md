...
- The linker refuses an anachronistic link (issue #319): a hit on a case decided in a later year than the citing treatise volume was published (`moml.book_info.year`). The case's year is `cap.cases.decision_year`, `legalhist.code_reporter.decision_year`, or for the English Reports `murrell_year`, falling back to `er_year`. The same year is not anachronistic, and an unknown year keeps the link. A refused hit is passed over and the cascade continues; if nothing else links, the row is `no_match` with tier `us_anachronistic` or `uk_anachronistic`. `db/queries/anachronistic-links.sql` measures it.
