...
func TestOCRDigitDetector_MotivatingCase(t *testing.T) {
	...
	// The abbreviation recorded is the one that appeared in the OCR, exactly as
	// for every other detector. Normalizing "F1ed." to "Fed." here would assert
	// a reading of the text that belongs to legalhist.whitelist, not to the
	// detector; the whitelist has to carry the spelling for this to link.
	assert.Equal(t, "F1ed.", cites[0].ReporterAbbr)
}

// TestOCRDigitDetector_Variants runs the shapes of corruption actually observed
// in the OCR through the detector, and pins down that ReporterAbbr is always
// the scanned spelling.
func TestOCRDigitDetector_Variants(t *testing.T) {
	tests := []struct {
		...
		{"see 102 I1l. 315 for the rule", "102 I1l. 315", "I1l.", 102, 315},
		{"see 123 Ma5ss. 286 for the rule", "123 Ma5ss. 286", "Ma5ss.", 123, 286},
		{"see 14 M3ich. 103 for the rule", "14 M3ich. 103", "M3ich.", 14, 103},
		{"see 181 F1ed. 3 for the rule", "181 F1ed. 3", "F1ed.", 181, 3},
		{"see 1 M1arsh. 525 for the rule", "1 M1arsh. 525", "M1arsh.", 1, 525},
		{"see 12 M1od. 331 for the rule", "12 M1od. 331", "M1od.", 12, 331},
		{"see 38 M3d. 459 for the rule", "38 M3d. 459", "M3d.", 38, 459},
		{"see 7 B1ing. 349 for the rule", "7 B1ing. 349", "B1ing.", 7, 349},
	}
...
// The edition suffix matters here -- the digit in "4th ed." follows a space, so
// it is not letter-flanked and must not trip the detector.
