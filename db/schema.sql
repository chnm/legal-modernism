...
--
-- Name: opinion_citations; Type: SCHEMA; Schema: -; Owner: -
--
...
--
-- Name: SCHEMA opinion_citations; Type: COMMENT; Schema: -; Owner: -
--
...
--
-- Name: chnm_revoke_role_from_all(text, boolean); Type: PROCEDURE; Schema: sys_admin; Owner: -
--

CREATE PROCEDURE sys_admin.chnm_revoke_role_from_all(IN p_role text, IN p_dry_run boolean DEFAULT true)
    LANGUAGE plpgsql
    AS $$
DECLARE
    r           RECORD;
...
BEGIN
    ...
    LOOP
        sql_stmt := format(
            ...
            r.privilege_type, r.object_schema, r.object_name, p_role
        );
        RAISE NOTICE '[sequence] %', sql_stmt;
        IF NOT p_dry_run THEN EXECUTE sql_stmt; END IF;
        obj_count := obj_count + 1;
    END LOOP;
    ...
    LOOP
        ...
        RAISE NOTICE '[function] %', sql_stmt;
        IF NOT p_dry_run THEN EXECUTE sql_stmt; END IF;
        obj_count := obj_count + 1;
    END LOOP;
    ...
    LOOP
        sql_stmt := format(
            ...
            r.privilege_type, r.nspname, p_role
        );
        RAISE NOTICE '[schema] %', sql_stmt;
        IF NOT p_dry_run THEN EXECUTE sql_stmt; END IF;
        obj_count := obj_count + 1;
    END LOOP;
    ...
    LOOP
        sql_stmt := format(
            ...
            r.privilege_type, r.datname, p_role
        );
        RAISE NOTICE '[database] %', sql_stmt;
        IF NOT p_dry_run THEN EXECUTE sql_stmt; END IF;
        obj_count := obj_count + 1;
    END LOOP;
...
--
-- Name: COLUMN opinions.id; Type: COMMENT; Schema: cap; Owner: -
--
...
--
-- Name: opinions_id_seq; Type: SEQUENCE; Schema: cap; Owner: -
--

ALTER TABLE cap.opinions ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME cap.opinions_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE moml.editions (
    ...
    derivative boolean DEFAULT false NOT NULL,
...
--
-- Name: COLUMN editions.work_id; Type: COMMENT; Schema: moml; Owner: -
--

COMMENT ON COLUMN moml.editions.work_id IS 'The work this edition belongs to.';


--
-- Name: COLUMN editions.derivative; Type: COMMENT; Schema: moml; Owner: -
--

COMMENT ON COLUMN moml.editions.derivative IS 'True when the edition is an abridgment, analysis, question book, supplement, adaptation or translation of its work rather than an edition of it.';
...
--
-- Name: works; Type: TABLE; Schema: moml; Owner: -
--

CREATE TABLE moml.works (
    work_id integer NOT NULL,
    author text,
    title text NOT NULL
);
...
--
-- Name: TABLE works; Type: COMMENT; Schema: moml; Owner: -
--

COMMENT ON TABLE moml.works IS 'A treatise across all its editions, including later editors'' revisions, translations, and works derived from it (abridgments, analyses, question books, supplements); every edition belongs to one. See db/queries/work-candidates.sql.';


--
-- Name: COLUMN works.author; Type: COMMENT; Schema: moml; Owner: -
--

COMMENT ON COLUMN moml.works.author IS 'First author of the work''s principal edition (its earliest edition that is not derivative); NULL for an anonymous work.';


--
-- Name: COLUMN works.title; Type: COMMENT; Schema: moml; Owner: -
--

COMMENT ON COLUMN moml.works.title IS 'Main title of the work''s principal edition.';


--
-- Name: works_work_id_seq; Type: SEQUENCE; Schema: moml; Owner: -
--

ALTER TABLE moml.works ALTER COLUMN work_id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME moml.works_work_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
--
-- Name: citation_links; Type: TABLE; Schema: opinion_citations; Owner: -
--
...
--
-- Name: TABLE citation_links; Type: COMMENT; Schema: opinion_citations; Owner: -
--
...
--
-- Name: citations_unlinked; Type: TABLE; Schema: opinion_citations; Owner: -
--
...
--
-- Name: TABLE citations_unlinked; Type: COMMENT; Schema: opinion_citations; Owner: -
--
...
--
-- Name: case_case_citations; Type: MATERIALIZED VIEW; Schema: opinion_citations; Owner: -
--

CREATE MATERIALIZED VIEW opinion_citations.case_case_citations AS
 SELECT cu.cap_case AS citing_case_id,
        CASE
            WHEN (cl.cap_case_id IS NOT NULL) THEN 'cap'::text
            WHEN (cl.er_case_id IS NOT NULL) THEN 'er'::text
            WHEN (cl.code_reporter_id IS NOT NULL) THEN 'code'::text
...
--
-- Name: MATERIALIZED VIEW case_case_citations; Type: COMMENT; Schema: opinion_citations; Owner: -
--
...
--
-- Name: COLUMN case_case_citations.citing_case_id; Type: COMMENT; Schema: opinion_citations; Owner: -
--
...
--
-- Name: COLUMN case_case_citations.source; Type: COMMENT; Schema: opinion_citations; Owner: -
--
...
--
-- Name: COLUMN case_case_citations.cite_count; Type: COMMENT; Schema: opinion_citations; Owner: -
--
...
--
-- Name: COLUMN case_case_citations.opinion_count; Type: COMMENT; Schema: opinion_citations; Owner: -
--
...
--
-- Name: COLUMN case_case_citations.pincite_only; Type: COMMENT; Schema: opinion_citations; Owner: -
--
...
--
-- Name: COLUMN case_case_citations.self_cite; Type: COMMENT; Schema: opinion_citations; Owner: -
--
...
--
-- Name: table_row_counts; Type: VIEW; Schema: stats; Owner: -
--

CREATE VIEW stats.table_row_counts AS
 WITH tbl AS (
         SELECT tables.table_schema,
            tables.table_name
           FROM information_schema.tables
          WHERE (((tables.table_name)::name !~~ 'pg_%'::text) AND ((tables.table_schema)::name <> ALL (ARRAY['information_schema'::name, 'stats'::name])))
        )
 SELECT table_schema,
    table_name,
    (((xpath('/row/c/text()'::text, query_to_xml(format('select count(*) as c from %I.%I'::text, table_schema, table_name), false, true, ''::text)))[1])::text)::integer AS row_count
   FROM tbl
  ORDER BY table_schema, (((xpath('/row/c/text()'::text, query_to_xml(format('select count(*) as c from %I.%I'::text, table_schema, table_name), false, true, ''::text)))[1])::text)::integer DESC;
...
--
-- Name: opinions opinions_pkey; Type: CONSTRAINT; Schema: cap; Owner: -
--
...
--
-- Name: works works_pkey; Type: CONSTRAINT; Schema: moml; Owner: -
--

ALTER TABLE ONLY moml.works
...
--
-- Name: citation_links citation_links_pkey; Type: CONSTRAINT; Schema: opinion_citations; Owner: -
--

ALTER TABLE ONLY opinion_citations.citation_links
    ADD CONSTRAINT citation_links_pkey PRIMARY KEY (citation_id);


--
-- Name: citations_unlinked citations_unlinked_pkey; Type: CONSTRAINT; Schema: opinion_citations; Owner: -
--

ALTER TABLE ONLY opinion_citations.citations_unlinked
    ADD CONSTRAINT citations_unlinked_pkey PRIMARY KEY (id);
...
--
-- Name: opinions_case_idx; Type: INDEX; Schema: cap; Owner: -
--

CREATE INDEX opinions_case_idx ON cap.opinions USING btree ("case");
...
--
-- Name: editions_work_id_idx; Type: INDEX; Schema: moml; Owner: -
--

CREATE INDEX editions_work_id_idx ON moml.editions USING btree (work_id);
...
--
-- Name: case_case_citations_cap_idx; Type: INDEX; Schema: opinion_citations; Owner: -
--

CREATE INDEX case_case_citations_cap_idx ON opinion_citations.case_case_citations USING btree (cap_case_id) WHERE (cap_case_id IS NOT NULL);


--
-- Name: case_case_citations_code_idx; Type: INDEX; Schema: opinion_citations; Owner: -
--

CREATE INDEX case_case_citations_code_idx ON opinion_citations.case_case_citations USING btree (code_reporter_id) WHERE (code_reporter_id IS NOT NULL);


--
-- Name: case_case_citations_er_idx; Type: INDEX; Schema: opinion_citations; Owner: -
--

CREATE INDEX case_case_citations_er_idx ON opinion_citations.case_case_citations USING btree (er_case_id) WHERE (er_case_id IS NOT NULL);


--
-- Name: case_case_citations_stub_idx; Type: INDEX; Schema: opinion_citations; Owner: -
--

CREATE INDEX case_case_citations_stub_idx ON opinion_citations.case_case_citations USING btree (stub_cite) WHERE (stub_cite IS NOT NULL);


--
-- Name: case_case_citations_uq; Type: INDEX; Schema: opinion_citations; Owner: -
--

CREATE UNIQUE INDEX case_case_citations_uq ON opinion_citations.case_case_citations USING btree (citing_case_id, cap_case_id, er_case_id, code_reporter_id, stub_cite) NULLS NOT DISTINCT;


--
-- Name: citation_links_cite_normalized_idx; Type: INDEX; Schema: opinion_citations; Owner: -
--

CREATE INDEX citation_links_cite_normalized_idx ON opinion_citations.citation_links USING btree (cite_normalized);


--
-- Name: citations_unlinked_cap_case_idx; Type: INDEX; Schema: opinion_citations; Owner: -
--

CREATE INDEX citations_unlinked_cap_case_idx ON opinion_citations.citations_unlinked USING btree (cap_case);


--
-- Name: citations_unlinked_reporter_abbr_idx; Type: INDEX; Schema: opinion_citations; Owner: -
--

CREATE INDEX citations_unlinked_reporter_abbr_idx ON opinion_citations.citations_unlinked USING btree (reporter_abbr);


--
-- Name: citations_unlinked_uq; Type: INDEX; Schema: opinion_citations; Owner: -
--

CREATE UNIQUE INDEX citations_unlinked_uq ON opinion_citations.citations_unlinked USING btree (cap_opinion, COALESCE(volume, '-1'::integer), reporter_abbr, page, COALESCE(year, '-1'::integer));


--
-- Name: idx_citation_links_cap; Type: INDEX; Schema: opinion_citations; Owner: -
--

CREATE INDEX idx_citation_links_cap ON opinion_citations.citation_links USING btree (cap_case_id) WHERE (cap_case_id IS NOT NULL);


--
-- Name: idx_citation_links_code; Type: INDEX; Schema: opinion_citations; Owner: -
--

CREATE INDEX idx_citation_links_code ON opinion_citations.citation_links USING btree (code_reporter_id) WHERE (code_reporter_id IS NOT NULL);


--
-- Name: idx_citation_links_er; Type: INDEX; Schema: opinion_citations; Owner: -
--

CREATE INDEX idx_citation_links_er ON opinion_citations.citation_links USING btree (er_case_id) WHERE (er_case_id IS NOT NULL);


--
-- Name: idx_citation_links_status; Type: INDEX; Schema: opinion_citations; Owner: -
--

CREATE INDEX idx_citation_links_status ON opinion_citations.citation_links USING btree (status);


--
-- Name: idx_citation_links_stub; Type: INDEX; Schema: opinion_citations; Owner: -
--

CREATE INDEX idx_citation_links_stub ON opinion_citations.citation_links USING btree (stub_cite) WHERE (stub_cite IS NOT NULL);
...
--
-- Name: editions editions_work_id_fkey; Type: FK CONSTRAINT; Schema: moml; Owner: -
--
...
--
-- Name: citation_links citation_links_cap_case_id_fkey; Type: FK CONSTRAINT; Schema: opinion_citations; Owner: -
--
...
--
-- Name: citation_links citation_links_citation_id_fkey; Type: FK CONSTRAINT; Schema: opinion_citations; Owner: -
--

ALTER TABLE ONLY opinion_citations.citation_links
    ADD CONSTRAINT citation_links_citation_id_fkey FOREIGN KEY (citation_id) REFERENCES opinion_citations.citations_unlinked(id);


--
-- Name: citation_links citation_links_code_reporter_id_fkey; Type: FK CONSTRAINT; Schema: opinion_citations; Owner: -
--
...
--
-- Name: citation_links citation_links_er_case_id_fkey; Type: FK CONSTRAINT; Schema: opinion_citations; Owner: -
--
...
--
-- Name: citations_unlinked citations_unlinked_cap_case_fkey; Type: FK CONSTRAINT; Schema: opinion_citations; Owner: -
--

ALTER TABLE ONLY opinion_citations.citations_unlinked
    ADD CONSTRAINT citations_unlinked_cap_case_fkey FOREIGN KEY (cap_case) REFERENCES cap.cases(id);


--
-- Name: citations_unlinked citations_unlinked_cap_opinion_fkey; Type: FK CONSTRAINT; Schema: opinion_citations; Owner: -
--

ALTER TABLE ONLY opinion_citations.citations_unlinked
    ADD CONSTRAINT citations_unlinked_cap_opinion_fkey FOREIGN KEY (cap_opinion) REFERENCES cap.opinions(id);
