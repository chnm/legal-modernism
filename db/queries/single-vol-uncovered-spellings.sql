...
--   \b + QuoteMeta(spelling) with each space replaced by [\s.]* + \w*[.,]*\s+\d{1,4}
...
-- the reporter the spelling maps to, because the \w* stem lets one reporter's
-- detector record another reporter's spelling ("Barnes" from a "Barn" detector).
--
...
rx AS (
  SELECT abbr,
         '^' || replace(regexp_replace(abbr, '([.^$*+?()\[\]{}|\\])', '\\\1', 'g'), ' ', '[\s.]*') || '\w*[.,]*$' AS re
  FROM patterns
),
wl AS (
