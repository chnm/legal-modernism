...
// TestRangeIndexLongVolumeFinalCase pins the one place maxSpanPages must not
// apply: a case whose length the source records is bounded by that length, not
// by the constant, even with no successor cite to confirm it. 671 CAP cases are
// volume-final in citation space and longer than maxSpanPages, and capping them
// would report their real interior pages as us_page_gap.
func TestRangeIndexLongVolumeFinalCase(t *testing.T) {
	ix := newRangeIndex([]citations.CaseSpan[int64]{
		{Cite: "1 Test 10", ID: 10, Length: 5},
		{Cite: "1 Test 20", ID: 20, Length: 120}, // volume-final, and genuinely long
	})

	id, outcome := ix.lookup("1", "Test", 100)
	assert.Equal(t, rangeHit, outcome, "a recorded length is not capped at maxSpanPages")
	assert.Equal(t, int64(20), id)

	id, outcome = ix.lookup("1", "Test", 139) // 20 + 120 - 1
	assert.Equal(t, rangeHit, outcome, "the case owns its last page")
	assert.Equal(t, int64(20), id)

	_, outcome = ix.lookup("1", "Test", 140)
	assert.Equal(t, rangeGap, outcome, "but not one page more")
}
